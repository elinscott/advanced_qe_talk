...
\begin{frame}{Core theory}
   \hbox{
      ...
      \begin{minipage}{0.45\textwidth}
         \centering
         \only<1>{
            \includegraphics[height=0.7\textheight]{figures/golze_pes.jpg}
...
\begin{frame}{Core theory}
   ...
   \begin{align*}
      E_\mathsf{Koopmans}[\rho,\only<4>{\textcolor{red}}{\{f_i\}}, \only<5>{\textcolor{red}}{\{\alpha_i\}}]
      = {E_{DFT}[\rho]}
      + \sum_i
      \only<5>{\textcolor{red}}
      {\alpha_i}
...
\begin{frame}{Orbital-density-dependence}
   ...
   \onslide<3->{
      ...
      \begin{equation*}
         v^\mathsf{KI}_{\textcolor{red}{i}}/\alpha_i = - E_{\mathsf{H}}\left[\textcolor{red}{n_{i}}\right]
         + E_{\mathsf{xc}}\left[\rho\right]
         - E_{\mathsf{xc}}\left[\rho-\textcolor{red}{n_{i}}\right]
         - \int d\mathbf{r'}
...
\begin{frame}{Orbital-density-dependence}
   ...
   \begin{description}[<+(1)->]
      \item[outer loop:] $\psi^{(n+1)}_i = \psi_i^{(n)} + \Delta_i$
      \item[inner loop:] $\psi^{(n+1)}_i = \sum_{j}U_{ij}\psi_j^{(n)}$
   \end{description}
...
\begin{frame}{Importance of localisation}
   ...
   \only<2-3>{
      In the bulk limit for one cell $\Delta E = E(N-\delta) - E(N)$
   }
...
\begin{frame}{Screening}
   ...
   \begin{overlayarea}{\textwidth}{0.65\textheight}
      \begin{center}
         ...
         \only<3>{\includegraphics[width=0.5\textwidth]{figures/alpha_calc/fig_alpha_calc_step_2.pdf}}
         \only<4>{\includegraphics[width=0.5\textwidth]{figures/alpha_calc/fig_alpha_calc_step_3.pdf}}
         \only<5->{\includegraphics[width=0.5\textwidth]{figures/alpha_calc/fig_alpha_calc_step_4.pdf}}
      \end{center}
   ...
   \begin{overlayarea}{\textwidth}{0.3\textheight}
      \only<3-5>{
         \begin{equation*}
            ...
            \braopket{\varphi_i}{\hat h^\mathsf{DFT} + \alpha_i \hat v^\mathsf{Koopmans}}{\varphi_i}
            =
            \left.\frac{dE^\mathsf{Koopmans}}{df_i}\right|_{f_i = s}
         \end{equation*}
      }
      \only<6->{Given this, how to work out the ideal $\alpha$?}
      \only<7->{
         \begin{equation*}
